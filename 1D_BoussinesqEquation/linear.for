C	ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)
C	ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ
C	ΜΕ ΤΗ ΜΕΘΟΔΟ ΤΩΝ ΠΕΠΕΡΑΣΜΕΝΩΝ ΣΤΟΙΧΕΙΩΝ
C	
C	ΤΡΟΠΟΙ ΠΡΟΣΣΕΓΓΙΣΗΣ ΣΤΟ ΤΕΛΟΣ ΤΗΣ ΠΕΡΙΟΧΗΣ ΜΕΛΕΤΗΣ
C	1. ΠΕΠΕΡΑΣΜΕΝΟ ΣΤΟΙΧΕΙΟ
C	2. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=(ri/r)**a
C	3. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=exp{a*(ri-r)}
C
C
C	Τα δεδομένα εισάγονται στο αρχείο 'linear.in'
C
C	Οι αδιάστατες τιμές του ύψους της υπόγειας στάθμης
C	εκτυπώνονται στο αρχείο 'lineara.out'
C
C	Οι αδιάστατες τιμές της παροχής που εισέρχεται στον υδροφορέα
C	εκτυπώνονται στο αρχείο 'linearb.out'
C
C	Οι πραγματικές τιμές του ύψους της υπόγειας στάθμης
C	εκτυπώνονται στο αρχείο 'linearc.out'
C
C	Οι αδιάστατες τιμές του ύψους της υπόγειας στάθμης
C	στους IX1,IX2,IX3 κόμβους της περιοχής
C	εκτυπώνονται στο αρχείο 'lineard.out'
C
C	Οι τιμές της συγκέντρωσης της ουσίας
C	εκτυπώνονται στο αρχείο 'POLLUTION.out'
C
C
C	Οι βασικότερες μεταβλητές που χρησιμοποιούνται στο πρόγραμμα είναι:
C	H: Το αδιάστατο ύψος της υπόγειας στάθμης στην προηγούμενη
C	   χρονική στιγμή - Αρχική συνθήκη
C	A,B,C,D: Πίνακες που χρησιμοποιούνται στον αλγόριθμο του Thomas
C	DX: Το αδιάστατο χωρικό βήμα της περιοχής μελέτης
C	DT: Το αδιάστατο χρονικό βήμα μελέτης του φαινομένου
C	N: Ο αριθμός των στοιχείων με τη χρήση του ημιάπειρου στοιχείου
C	NK: Ο αριθμός των ενδιάμεσων κόμβων του δικτύου
C	NB: Ο αριθμός των στοιχείων χωρίς τη χρήση του ημιάπειρου στοιχείου
C	NBK: Ο αριθμός των ενδιάμεσων κόμβων του δικτύου
C	NNJ: Το χωρικό βήμα εκτύπωσης των αποτελεσμάτων
C	I: Ο χρονικός δείκτης μεταβολής
C	NT: Ο μέγιστος χρονικός δείκτης μεταβολής
C	NN: Το χρονικό βήμα εκτύπωσης των αποτελεσμάτων
C	TIME: Αδιάστατος χρόνος
C	RK(m/day): Υδραυλική αγωγιμότητα του εδάφους
C	RD(m): Προσεγγιστικό μέσο βάθος ροής
C	RS: Ειδική σε νερό απόδοση
C	HO(m): Βάθος νερού στην τάφρο - Οριακή συνθήκη
C	HA(m): Βάθος νερού στο τέλος της περιοχής μελέτης - Οριακή συνθήκη
C	HOD: Αδιάστατο βάθος νερού στην τάφρο - Οριακή συνθήκη
C
 ...
 200	FORMAT('ΑΡΧΕΙΟ ΕΞΟΔΟΥ ΑΠΟΤΕΛΕΣΜΑΤΩΝ - lineara.out'//
     $'ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)'/
     $'ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ'/
     $'ΚΑΘΩΣ ΚΑΙ ΤΗΣ ΕΞΙΣΩΣΗΣ ΤΗΣ ΜΕΤΑΦΟΡΑΣ ΜΑΖΑΣ'/
     $'ΜΕ ΤΗ ΜΕΘΟΔΟ ΤΩΝ ΠΕΠΕΡΑΣΜΕΝΩΝ ΣΤΟΙΧΕΙΩΝ'//
     $'ΤΡΟΠΟΙ ΠΡΟΣΣΕΓΓΙΣΗΣ ΣΤΟ ΤΕΛΟΣ ΤΗΣ ΠΕΡΙΟΧΗΣ ΜΕΛΕΤΗΣ'/
     $'1. ΠΕΠΕΡΑΣΜΕΝΟ ΣΤΟΙΧΕΙΟ - Z1(J)'/
     $'2. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=(ri/r)**a - Z2(J)'/
     $'3. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=exp{a*(ri-r)} - Z3(J)'//
     $'ΤΑ ΑΠΟΤΕΛΕΣΜΑΤΑ ΕΙΝΑΙ ΣΕ ΑΔΙΑΣΤΑΤΗ ΜΟΡΦΗ'//
     $'DX=',F10.5/'DT=',F10.5/'N=',I10/'NB=',I10/
     $'NT=',I10/'KMAX=',I10/'a=',F10.5/
     $'I=ΧΡΟΝΙΚΟΣ ΔΕΙΚΤΗΣ ΜΕΤΑΒΟΛΗΣ'/'J=ΧΩΡΙΚΟΣ ΔΕΙΚΤΗΣ ΜΕΤΑΒΟΛΗΣ'/
     $'TIME=ΑΔΙΑΣΤΑΤΟΣ ΧΡΟΝΟΣ'/////'ΑΠΟΤΕΛΕΣΜΑΤΑ')
 201	FORMAT('ΑΡΧΕΙΟ ΕΞΟΔΟΥ ΑΠΟΤΕΛΕΣΜΑΤΩΝ - linearb.out'//
     $'ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)'/
     $'ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ'//
     $'ΤΙΜΕΣ ΤΗΣ ΠΑΡΟΧΗΣ ΣΕ ΑΔΙΑΣΤΑΤΗ ΜΟΡΦΗ'//
     $4X,'I',5X,'Q1(I)',5X,'Q2(I)',5X,'Q3(I)')
 202	FORMAT('ΑΡΧΕΙΟ ΕΞΟΔΟΥ ΑΠΟΤΕΛΕΣΜΑΤΩΝ - linf1rc.out'//
     $'ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)'/
     $'ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ'/
     $'ΚΑΘΩΣ ΚΑΙ ΤΗΣ ΕΞΙΣΩΣΗΣ ΤΗΣ ΜΕΤΑΦΟΡΑΣ ΜΑΖΑΣ'/
     $'ΜΕ ΤΗ ΜΕΘΟΔΟ ΤΩΝ ΠΕΠΕΡΑΣΜΕΝΩΝ ΣΤΟΙΧΕΙΩΝ'//
     ...
     $'2. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=(ri/r)**a - Z2(J)'/
     $'3. ΗΜΙΑΠΕΙΡΟ ΣΤΟΙΧΕΙΟ ΟΠΟΥ f(ri/r)=exp{a*(ri-r)} - Z3(J)'//
     $'ΤΑ ΑΠΟΤΕΛΕΣΜΑΤΑ ΕΙΝΑΙ ΣΕ ΜΟΝΑΔΕΣ ΤΟΥ ΜΕΤΡΙΚΟΥ ΣΥΣΤΗΜΑΤΟΣ'//
     $'DX(m)=',F10.5/'DT(day)=',F10.5/'N=',I10/'NB=',I10/'NT=',I10/
     $'KMAX=',I10/'a=',F10.5/'RK(m/day)=',F10.5/'RS=',F10.5/
     $'HO(m)=',F10.5/'HA(m)=',F10.5/'RD(m)=',F10.5/
     $'I=ΧΡΟΝΙΚΟΣ ΔΕΙΚΤΗΣ ΜΕΤΑΒΟΛΗΣ'/'J=ΧΩΡΙΚΟΣ ΔΕΙΚΤΗΣ ΜΕΤΑΒΟΛΗΣ'/
     $'TIME=ΑΔΙΑΣΤΑΤΟΣ ΧΡΟΝΟΣ'/////'ΑΠΟΤΕΛΕΣΜΑΤΑ')
 203	FORMAT('ΑΡΧΕΙΟ ΕΞΟΔΟΥ ΑΠΟΤΕΛΕΣΜΑΤΩΝ - lineard.out'//
     $'ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)'/
     $'ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ'//
     $'ΤΙΜΕΣ ΤΗΣ ΥΠΟΓΕΙΑΣ ΣΤΑΘΜΗΣ ΣΤΟΝ ΚΟΜΒΟ ΙΧ'//
     $'ΚΟΜΒΟΣ ΙΧ=',I5,2X,'ΘΕΣΗ ΤΟΥ ΚΟΜΒΟΥ =',E8.2//
     $4X,'I',4X,'Ζ1(IΧ)',4X,'Ζ2(IΧ)',4X,'Ζ3(IΧ)',5X,'a4-Z2',6X,'a4-Z3')	
 204	FORMAT('ΑΡΧΕΙΟ ΕΞΟΔΟΥ ΑΠΟΤΕΛΕΣΜΑΤΩΝ - Polution.out'//
     $'ΕΠΙΛΥΣΗ ΤΗΣ ΓΡΑΜΜΙΚΟΠΟΙΗΜΕΝΗΣ(ΩΣ ΠΡΟΣ Η**2)'/
     $'ΚΑΙ ΑΔΙΑΣΤΑΤΗΣ ΕΞΙΣΩΣΗΣ ΤΟΥ BOUSSINESQ'/
     $'ΚΑΘΩΣ ΚΑΙ ΤΗΣ ΕΞΙΣΩΣΗΣ ΤΗΣ ΜΕΤΑΦΟΡΑΣ ΜΑΖΑΣ'/
     $'ΜΕ ΤΗ ΜΕΘΟΔΟ ΤΩΝ ΠΕΠΕΡΑΣΜΕΝΩΝ ΣΤΟΙΧΕΙΩΝ'//
     $'ΤΑ ΑΠΟΤΕΛΕΣΜΑΤΑ ΕΙΝΑΙ ΣΕ ΜΟΝΑΔΕΣ ΤΟΥ ΜΕΤΡΙΚΟΥ ΣΥΣΤΗΜΑΤΟΣ'/
     $'ΓΙΑ ΤΙΣ ΤΙΜΕΣ ΤΟΥ ΦΟΡΤΙΟΥ ΣΕ ΚΛΕΙΣΤΟ ΥΔΡΟΦΟΡΕΑ-PZ1(m)'/
     $'ΚΑΙ ΓΙΑ ΤΙΣ ΤΙΜΕΣ ΤΗΣ ΣΥΓΚΕΝΤΡΩΣΗΣΗ-PC1(mg/l)'//
...
C	Εισαγωγή οριακής συνθήκης στην τάφρο & αρχικής συνθήκης
C	*******************************************************
	IF(HO>HA)THEN
	HOD=1
	ELSE
...
C	Συντελεστές πρώτης εξίσωσης
C	***************************
	B(1)=2/DX+2*DX/3/DT
	C(1)=1/DX-DX/6/DT
	D(1)=(1/DX)*HOD+(2*DX/3/DT)*H(1)+(DX/6/DT)*H(2)

C	Συντελεστές των υπόλοιπων εξισώσεων
C	***********************************
	DO 20 J=2,NBK-1
	A(J)=1/DX-DX/6/DT
	B(J)=2/DX+2*DX/3/DT
...
C	Συντελεστές τελευταίας εξίσωσης
C	*******************************
	IF(METHOD==1) THEN
	A(NBK)=1/DX-DX/6/DT
...
C	Προετοιμασία για την επόμενη χρονική στιγμή
C	*******************************************
	IF(METHOD==1) Z1=V
	IF(METHOD==2) Z2=V
	IF(METHOD==3) Z3=V
...
C	Εκτύπωση των φορτίων σε αδιάστατες και πραγματικές τιμές
...
C	Υπολογισμός και εκτύπωση της παροχής σε αδιάστατες τιμές
	Q1=ABS(HOD-Z1(1))/DX
	Q2=ABS(HOD-Z2(1))/DX
	Q3=ABS(HOD-Z3(1))/DX
...
C	Εκτύπωση της υπόγειας στάθμης στον κόμβο IX
	WRITE(12,222)I,Z1(IX),Z2(IX),Z3(IX),XI2,XI3
	WRITE(19,*)Z1(IX)
	WRITE(20,*)Z2(IX)
	WRITE(21,*)Z3(IX)

C	Εκτύπωση των αποτελεσμάτων μεταφοράς μάζας
